...
def generate_log(pm4py_log,verbose=False):
    ...
    for trace in pm4py_log:
        ...
        for event in trace:
            ...
            new_event = Event(trace.attributes['concept:name'], event['concept:name'], event['time:timestamp'])
    ...
    count = 5
    if(verbose): # to print the entire log. momentanely disable expect for the first 5 rows
        ...
        for event in log:
            ...
            count -=1
            if count <= 0:
                break     # Print only the first 5 rows
...
def measure_pentland_process(pm4py_log, quiet=False, verbose=False):
    ...
    m_pentland_process = 10**(0.08*(exponent if exponent <= 3850 else 3850)) # cap at 1e+308 to avoid inf (from documentation occurs at exponent > 3853)
...
def perform_measurements(desired_measurements, log=None, pm4py_log=None, pa=None, quiet=True,verbose=False):
    ...
    if check_measure("pentland","all"):
        print("Pentland's task complexity")
    ...
    if check_measure("pentland_process","all"):
        print("Pentland's process complexity")
...
if __name__ == "__main__":
    ...
    print("Reading and preparing event log", args.file)
    base_filename = extract_base_filename(args.file)
    pm4py_log = generate_pm4py_log(args.file)
    ...
    if(args.measures):
        print("Starting performing measurements", args.file)
